...
int longestBitonicSubarray(int *input, int n) {
    ...
    for(int i=1;i<n;i++)
    {
      for(int j=0;j<i;j++) if(arr[i]>arr[j])up[i]=max(up[i],up[j]+1);
    }   
    //fo(i,n) cout<<up[i]<<" ";
    for(int i=n-2;i>=0;i--)
    {
      for(int j=i+1;j<n;j++) if(arr[i]>arr[j]) down[i]=max(down[i],down[j]+1);
    }
    //cout<<endl;
    //fo(i,n) cout<<down[i]<<" ";
    fo(i,n) up[i]+=down[i];
    return (*max_element(up,up+n) -1);
