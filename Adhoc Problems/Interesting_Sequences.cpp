...
int ans=INT_MAX;
int k,l;
void Find_answer(int arr[],int n,int equal,int max)
{   
    int an;
    if(equal>max) return;
    else
    {
        ...
        for(int i=0;i<n;i++)
        {
            if(arr[i]<equal) increase+=equal-arr[i];
            else 
            {
                decrease+=arr[i]-equal;
            }
          ...
          if(increase>=decrease) 
            {
                an=decrease*k;
                an+=(increase-decrease)*l;
                if(an<ans) ans=an;
            }
    }
    Find_answer(arr,n,equal+1,max);
}
int main() {
